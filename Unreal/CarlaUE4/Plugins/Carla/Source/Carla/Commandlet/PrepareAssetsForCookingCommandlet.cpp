...
void UPrepareAssetsForCookingCommandlet::GenerateMapPathsFile(
    ...
    const FString &PropsMapPath) const
{
  FString MapPathData;
...
void UPrepareAssetsForCookingCommandlet::GeneratePackagePathFile(const FString &PackageName) const
{
  FString SaveDirectory = FPaths::ProjectContentDir();
  FString FileName = FString("PackagePath.txt");
  FString PackageJsonFilePath = GetFirstPackagePath(PackageName);
  SaveStringTextToFile(SaveDirectory,
      FileName,
      PackageJsonFilePath,
      true);
}
    ...
    FString &MapDestPath)
{
  ...
  SaveWorld(AssetData, PackageName, MapDestPath, TEXT("PropsMap"));
